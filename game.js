...
window.Game = (function() {
  ...
  function showSurpriseSprite(s) {
    ...
    // Outer wrapper carries the user's rotation/scale/filter.
    // Inner element carries the animation (so it doesn't clobber the transform).
    const outer = document.createElement('div');
    outer.className = 'surprise-sprite-wrap';
    outer.style.position = 'absolute';
    outer.style.left = `${s.x - sprW / 2}px`;
    outer.style.top  = `${s.y - sprW / 2}px`;
    outer.style.width = `${sprW}px`;
    outer.style.pointerEvents = 'none';
    outer.style.zIndex = 6;
    if (s.transform && window.Transforms) {
      window.Transforms.applyTo(outer, s.transform);
    }
